...
@connect(({ componentType, loading }) => ({
  componentType,
  submitting: loading.effects['componentType/fetch'],
  loading: loading.models.componentType,
}))

@Form.create()
export default class ComponentType extends PureComponent {
  state = {
    id: 0,
    name: '',
    modalVisible: false,
  ...
  handleRefresh = () => {
    const { dispatch } = this.props;
    dispatch({ type: 'componentType/fetch', payload: {} });
  ...
  handleEdit = (_id, fields) => {
    this.props.dispatch({
      type: _id > 0 ? 'componentType/update' : 'componentType/add',
      payload: {
        id: _id,
        name: fields.name,
      },
  ...
  handleDeleteComponentType = (record, e) => {
    e.preventDefault();
  ...
  render() {
    const { componentType: { data }, loading } = this.props;
      ...
      },
      {
        ...
        render: (text, record) => (
          <Fragment>
            ...
            <a href="#" onClick={e => this.handleDeleteComponentType(record, e)}>删除</a>
          </Fragment>
        ),
      },
    ];
